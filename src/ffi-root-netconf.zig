...
export fn ls_netconf_fetch_operation(
    ...
    if (ret.err) |ret_err| {
        ...
        // Copy as much of the error name / last error as fits in the caller-provided buffers
        // (sized via ls_netconf_fetch_operation_sizes). We deliberately bound the length with
        // @min and use copyForwards rather than @memcpy: this is an ffi boundary, and an
        // informational error name must never be able to abort the entire host process. A plain
        // @memcpy here asserts both equal lengths *and* non-aliasing in safe builds; if the size
        // and fetch calls ever disagree (or the source/destination unexpectedly relate), that
        // assertion turns a benign mismatch into a SIGABRT for the whole agent.
        const n = @min(operation_error.len, err_name.len);
        std.mem.copyForwards(u8, operation_error.*[0..n], err_name[0..n]);

        const last_error_n = @min(operation_last_error.len, ret.last_error.len);
        std.mem.copyForwards(u8, operation_last_error.*[0..last_error_n], ret.last_error[0..last_error_n]);
    } else {
        const dret = switch (ret.result) {
            .netconf => |r| r.?,
            else => {
        ...
        _ = bytes.ffiCopy(operation_result_raw.*, dret.result_raw_journal);
